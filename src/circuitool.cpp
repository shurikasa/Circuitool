...
po::parsed_options parse_args(int argc, char** argv,
  ...
  "  export gmsh [circuit-file] [geo-file] [data-folder]:\texport circuit file to .geo file format using non-default data folder\n"
  "\n\n"
  "Options");
  general.add_options()
...
void export_circuit_to_gmsh(const std::string & filename_circuit, const std::string & filename_geo, const std::string & data_folder,
  po::variables_map & options){
    ...
    circuit_exporter circuit(filename_circuit,data_folder);
    ...
    try{
      ...
      if(options.count("command") && options.count("subargs") ){
        ...
        if(command == "export" ){
          ...
          if(subargs.size() == 4){
            export_circuit_to_gmsh(subargs[1], subargs[2], subargs[3], options);
            return 0;
          }
        };
      }
